...
class FACILITY_DATAFilter(django_filters.FilterSet):
    ...
    Prouducts = django_filters.ModelMultipleChoiceFilter( 
        queryset=INDUSTRIAL_PRODUCTS.objects.all() 
    )
    class Meta:
        model = FACILITY_DATA
        fields = ['NAME','CEO_NAME','DETAILED_ADDRESS','PRIMARY_MOBILE','my__REGISTRY_NUMBER']
